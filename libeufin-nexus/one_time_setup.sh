...
if libeufin-cli users list &> /dev/null; then
	...
	libeufin-cli connections new-ebics-connection --ebics-url $EBICS_URL --host-id $EBICS_HOST --partner-id $EBICS_PARTNER --ebics-user-id $EBICS_USER $CONNECTION_NAME
	...
	echo "Nexus has been set up successfully"
fi
